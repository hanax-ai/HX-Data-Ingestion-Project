...
**15-Server Canonical Deployment (<internal-network>):**
...
│  ├─ <control-plane-server-01>.<domain>                      │
│  ├─ <control-plane-server-02>.<domain>                      │
│  └─ <control-plane-server-03>.<domain>                      │
│                                                               │
│  Application Services (4 servers)                            │
│  ├─ <app-server-01>.<domain>  (Acquisition)                 │
│  ├─ <app-server-02>.<domain>  (Indexing)                   │
│  ├─ <app-server-03>.<domain>  (Retrieval)                  │
│  └─ <app-server-04>.<domain>  (API Gateway)                │
│                                                               │
│  Data Services (4 servers)                                   │
│  ├─ <data-server-01>.<domain>  (Qdrant Master)             │
│  ├─ <data-server-02>.<domain>  (Qdrant Replica)            │
│  ├─ <data-server-03>.<domain>  (PostgreSQL)                │
│  └─ <data-server-04>.<domain>  (Redis)                     │
│                                                               │
│  AI Services (2 servers)                                     │
│  ├─ <llm-server-01>  (gpt-oss:20b)                          │
│  └─ <llm-server-02>  (phi3:3.8b)                            │
│                                                               │
│  Monitoring & Support (2 servers)                            │
│  ├─ <monitor-server-01>.<domain>                            │
│  └─ <monitor-server-02>.<domain>                            │
│                                                               │
...
> **Note**: Detailed server hostnames, IP addresses, and network configuration are maintained in secure operational documentation with restricted access. Contact the Infrastructure Team for deployment-specific details.
