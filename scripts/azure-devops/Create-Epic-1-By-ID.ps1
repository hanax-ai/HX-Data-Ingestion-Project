# Create Epic 1 using project ID instead of name

$organization = "hana-x"
$projectId = "5c89f61b-f983-446c-a387-7792315d77e2"  # HX-Citadel-Delta project ID
...
Write-Host "Creating Epic 1 using project ID..." -ForegroundColor Cyan

# Step 1: Verify project exists by ID
Write-Host "1. Verifying project by ID..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "❌ Project not found by ID: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}

# Step 2: Check work item types using project ID
Write-Host "`n2. Checking work item types..." -ForegroundColor Yellow

try {
    $witUri = "https://dev.azure.com/$organization/$projectId/_apis/wit/workitemtypes?api-version=7.1-preview.3"
...
try {
    ...
    $epicData = @{
        ...
        Method = "PAT Token - REST API by Project ID"
    }
    
    $epicData | ConvertTo-Json | Out-File "out/epic-1-success-by-id.json"
    Write-Host "Epic data saved to out/epic-1-success-by-id.json" -ForegroundColor Green
    ...
    Write-Host "Project: https://dev.azure.com/$organization/$($projectResponse.name)" -ForegroundColor Cyan
    Write-Host "Epic Board: https://dev.azure.com/$organization/$($projectResponse.name)/_boards/board/t/$($projectResponse.name)%20Team/Epics" -ForegroundColor Cyan
