# Create Epic 1 with proper PAT token
# Replace the PAT variable below with your new PAT that has Work Items (Write) scope

param(
    [string]$PatToken = "REPLACE_WITH_YOUR_NEW_PAT_TOKEN"
)

$organization = "hana-x"
$projectName = "Citadel"

if ($PatToken -eq "REPLACE_WITH_YOUR_NEW_PAT_TOKEN") {
    Write-Host "❌ Please provide a PAT token with Work Items (Write) scope" -ForegroundColor Red
    Write-Host "Usage: .\Create-Epic-1-With-PAT.ps1 -PatToken 'your-pat-token'" -ForegroundColor Yellow
    Write-Host "Or edit the script and replace the PatToken variable" -ForegroundColor Yellow
    Write-Host "`nCreate PAT at: https://dev.azure.com/$organization/_usersSettings/tokens" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "1. Verify PAT has Work Items (Read, Write) scope" -ForegroundColor White
    Write-Host "2. Check PAT is not expired" -ForegroundColor White
    Write-Host "3. Ensure you have permissions to create work items in the project" -ForegroundColor White
}
